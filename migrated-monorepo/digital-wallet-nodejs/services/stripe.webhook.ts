import Stripe from 'stripe';

const stripe = new Stripe(process.env.STRIPE_SECRET_KEY as string, {apiVersion: '2020-08-27'});

class StripeService {
    static async createCustomer(email: string): Promise<Stripe.Customer> {
        return await stripe.customers.create({email});
    }

    static async createPaymentMethod(type: string, card: Stripe.PaymentMethodCreateParams.Card1): Promise<Stripe.PaymentMethod> {
        return await stripe.paymentMethods.create({card});
    }

    static async attachPaymentMethodToCustomer(paymentMethodId: string, customerId: string): Promise<Stripe.PaymentMethod> {
        return await stripe.paymentMethods.attach(paymentMethodId, {customer: customerId});
    }

    static async listCustomerPaymentMethods(customerId: string): Promise<Stripe.ApiList<Stripe.PaymentMethod>> {
        return await stripe.paymentMethods.list({
            customer: customerId,
            type: 'card',
        });
    }

    static async createPaymentIntent(amount: number, currency: string, customerId: string, paymentMethodId: string): Promise<Stripe.PaymentIntent> {
        return await stripe.paymentIntents.create({
            amount,
            currency,
            customer: customerId,
            payment_method: paymentMethodId,
            confirm: true,
        });
    }
}

export default StripeService;