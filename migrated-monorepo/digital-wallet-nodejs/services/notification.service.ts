...
class NotificationService {
    ...
    private baseTemplate: Handlebars.TemplateDelegate = Handlebars.compile('');
    ...
    private async sendEmail(to: string, subject: string, templateName: string, context: any): Promise<void> {
        try {
    ...
    async notifyEmailVerification(user: any, verificationLink: string): Promise<boolean> {
        try {
    ...
    async notifyLogin(user: any, loginTime: string, loginLocation: string): Promise<void> {
        try {
            await this.sendEmail(
                user.email,
    ...
    async notifyTransfer(fromUserId: string, toUserId: string, amount: number, transactionId: string, fromBalance: number, toBalance: number): Promise<void> {
        try {
    ...
    async notifyWithdrawal(userId: string, amount: number, transactionId: string, withdrawalStatus: string, withdrawalMethod: string, failureReason: string | null = null): Promise<void> {
        try {
            const user = await User.findById(userId);
            if (!user) throw new Error('User not found');
